...
class Ommatidium:
    ...
    def copy(self):
        ...
        return Ommatidium(
            self.position.copy(),
            self.direction.copy(),
            self.acceptanceAngle,
            self.focalpointOffset,
        )
...
def configureFunctions(eyeRenderer):
    ...
    eyeRenderer.setCameraPose.argtypes = [
        c_float
    ] * 6  # pos x, y, z, rotation about x, y, z
...
def setRenderSize(eyeRenderer, width, height):
    ...
    eyeRenderer.getFramePointer.restype = ndpointer(
        dtype=c_ubyte, shape=(height, width, 4)
    )
...
def setOmmatidiaFromOmmatidiumList(eyeRenderer, ommList):
    ...
    packetList = [
        c_ommatidiumPacket(
            *[float(n) for n in o.position],
            *[float(n) for n in o.direction],
            o.acceptanceAngle,
            o.focalpointOffset
        )
        for o in ommList
...
def gotoFirstCompoundEye(eyeRenderer):
    ...
    for i in range(camCount):
        ...
        if eyeRenderer.isCompoundEyeActive():
            ...
            print(
                "\twith this many ommatidia:",
                eyeRenderer.getCurrentEyeOmmatidialCount(),
            )
            break
...
def getIcoOmmatidia():
    ...
    icoPoints = icoPoints + [
        [math.cos(a) * planarDistance, verticalOffset, math.sin(a) * planarDistance]
        for a in angles
    ]
    ...
    return [
        Ommatidium(np.zeros(3), p, oneSteradianAcceptanceAngle, 0.0) for p in icoPoints
    ]
