...
def plane_square(vn: np.array, vt: np.array, mf=2) -> np.array:
    """
    ...
    pl["x"] = [0.5, 0.5, -0.5, -0.5]
    pl["y"] = [0.5, -0.5, 0.5, -0.5]
    pl["z"] = [0, 0, 0, 0]
    ...
    theta = np.arccos(
        vn @ np.array([0, 0, 1]) / (np.linalg.norm(vn) * 1)
    )  # angle between vn and z-axis
    phi = (
        np.arccos(vxy @ np.array([1, 0, 0]) / (np.linalg.norm(vxy) * 1)) + np.pi
    )  # angle between vxy and x-axis
    Ry = np.array(
        [
